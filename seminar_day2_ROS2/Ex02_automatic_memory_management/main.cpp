...
// 생성자
    LidarData()
    {
        std::cout << "[Data] 라이다 데이터 생성\n";
    }
// 소멸자
    ~LidarData()
    {
        std::cout << "[Data] 라이다 데이터 소멸\n";
    }
// 멤버함수
    void info()
    {
        std::cout << "장애물 거리: 1.5m\n";
    }
};
...
// --- [unique_ptr 예제] ---
    ...
    // std::unique_ptr<MotorDriver> error_ptr = motor; // [ERROR] 복사 불가능
    std::unique_ptr<MotorDriver> new_owner = std::move(motor); // 소유권 이전 가능
    if (!motor) 
    {
        std::cout << "기존 motor 포인터는 이제 빈 껍데기!\n";
    }
...
// 모든 main 함수가 끝나면 raw_data와 new_owner가 자동으로 해제됨